#!/usr/bin/env bash
# Verify Phase 10 graphics analysis environment
set -euo pipefail

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
LABS="$REPO/labs/phase-10"
PASS=0
FAIL=0
WARN=0
...
if [ -n "${KERNEL_SRC:-}" ] && [ -f "$KERNEL_SRC/drivers/gpu/drm/amdgpu/amdgpu_drv.c" ] 2>/dev/null; then
	...
	warn "KERNEL_SRC not set — see resources/phase-10.md"
fi
...
echo "Environment ready for Phase 10."
