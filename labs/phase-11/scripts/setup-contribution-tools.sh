#!/usr/bin/env bash
# Verify Phase 11 upstream contribution environment
set -euo pipefail

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
LABS="$REPO/labs/phase-11"
PASS=0
FAIL=0
WARN=0
...
if [ -n "${KERNEL_SRC:-}" ] && [ -f "$KERNEL_SRC/scripts/checkpatch.pl" ] 2>/dev/null; then
	...
	warn "KERNEL_SRC not set — see resources/phase-11.md"
fi
...
echo "Environment ready for Phase 11."
