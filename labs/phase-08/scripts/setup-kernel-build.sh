#!/usr/bin/env bash
# Verify Phase 8 kernel build environment
set -euo pipefail

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
LABS="$REPO/labs/phase-08"
PASS=0
FAIL=0
WARN=0
...
if [ -n "${KERNEL_SRC:-}" ] && [ -d "$KERNEL_SRC" ]; then
	...
	warn "KERNEL_SRC not set or missing — clone kernel before Module 8.2 (see resources/phase-08.md)"
fi
...
echo "Environment ready for Phase 8."
echo "Read labs/phase-08/checklists/vm-safety-checklist.md before booting a custom kernel."
