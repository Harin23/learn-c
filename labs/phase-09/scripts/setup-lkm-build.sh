#!/usr/bin/env bash
# Verify Phase 9 LKM lab environment
set -euo pipefail

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
LABS="$REPO/labs/phase-09"
PASS=0
FAIL=0
WARN=0
...
echo ""
echo "=== Phase 8 toolchain (required for modules) ==="
if [ -x "$REPO/labs/phase-08/scripts/setup-kernel-build.sh" ]; then
	if "$REPO/labs/phase-08/scripts/setup-kernel-build.sh" >/dev/null 2>&1; then
		ok "Phase 8 kernel build tools available"
	else
		warn "Phase 8 toolchain incomplete — run verify-phase-08.sh"
	fi
else
	warn "Phase 8 setup script not found"
fi
...
if [ -n "${KERNEL_SRC:-}" ] && [ -f "$KERNEL_SRC/Makefile" ]; then
	...
	if make -C "$KERNEL_SRC" M="$LABS/hello-lkm" modules >/dev/null 2>&1; then
		...
		warn "hello-lkm build failed — check KERNEL_SRC and Phase 8 tree"
	fi
else
	warn "KERNEL_SRC not set — set before building modules (see resources/phase-09.md)"
fi
...
echo "Environment ready for Phase 9."
echo "Merge learn-c-modules.config into VM kernel before Module 9.1."
echo "Read labs/phase-09/checklists/module-vm-safety-checklist.md before insmod."
