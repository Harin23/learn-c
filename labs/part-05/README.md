...
Hands-on starter code for Part 5 modules.
...
```bash
./scripts/verify-part-05.sh
```

Use `strace` extensively in this part:

```bash
strace -f -e trace=process,network,file ./your_program
```

Prerequisite: [Part 4 exit gate](../../part-04-linux-user-space/README.md#part-exit-gate).
